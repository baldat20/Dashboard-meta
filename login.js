...
module.exports = async (req, res) => {
  try {
    ...
    for (let row of analistas.data.values || []) {
      if (String(row[1]).trim() === String(usuario).trim() && String(row[2]).trim() === String(senha).trim()) {
        return res.json({
          tipo: "analista",
          nome: row[0] || "",
          TMA: row[4] || "",
    ...
    for (let row of auxiliares.data.values || []) {
      if (String(row[0]).trim() === String(usuario).trim() && String(row[1]).trim() === String(senha).trim()) {
        return res.json({
          tipo: "auxiliar",
          nome: row[0] || "",
          EFICIENCIA: row[2] || "",
